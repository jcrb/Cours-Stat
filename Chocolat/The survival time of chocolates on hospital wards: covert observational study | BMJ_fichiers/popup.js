...
Drupal.behaviors.popup = function(context){
  ...
  //enabling login popup -added by Zoe A
  enableLoginBar();
  // Process menu leaves
...
function enableLoginBar(){
    ...
    $('div#status a').bind('click',function(e){	
            e.stopPropagation();
    ...
    $(document).click(function(event) { 
      ...
      //alert($(event.target).parents().index($('#sign-in-0-active')) );
     
     
      
      if($(event.target).parents().index($('#sign-in-0-active')) == -1 && $(event.target).parents().index($('#status')) == -1) {
            if($('.popup-element-body').is(":visible")) {
                $('.popup-element-body').hide();
            }
      } 
    });
   
 
  $('input#edit-name').live('click',function(e){     
     ...
     $('input#edit-pass').live('click',function(e){    
        if(jQuery.trim($('input#edit-name').val()) == 'Username'){
           $(this).val('');
        }
      });
...
function initiatLoginPopUp($this){
    ...
    var popUpId = $this.parents('div').attr('id')+ "-active";
    
    if($('div#'+popUpId+' .popup-element-body').is(':visible')){
    	
        $('div#'+popUpId+' .popup-element-body').hide();
    }else{
    	
        $('div.popup-element-body').hide();
    	var offset = $($this).find('button').offset();
        var winWidth = $(window).width();
        var setX = offset.left;
        var setY = offset.top + $($this).outerHeight();
        $('div#'+popUpId+' .popup-element-origin').css({
            top: setY,
            left: setX
          });
           	
        $('div#'+popUpId+' .popup-element-body').show();
        if($('div#'+popUpId+' .popup-element-body').hasClass('ajax')){
          var url = $('div#'+popUpId+' .popup-element-body div.popup-ahah-placeholder a').attr('href');
          $('div#'+popUpId+' .popup-element-body div.inner').load(url,function(){
              $('input#edit-pass').attr('value','*****');
              $('input#edit-name').attr('value','Username');
           });
        }
    }
    return false;
